...
// } else {
  ...
  try {
    ...
    $sqlMensajes = "SELECT * FROM mensajes WHERE sender_id = ? AND receiver_id = ?;";

    $stmtTablaMensajes = mysqli_stmt_init($conn);
    mysqli_stmt_prepare($stmtTablaMensajes, $sqlMensajes);
    mysqli_stmt_bind_param($stmtTablaMensajes, "ii", $userLogged['user_id'], $idAmigo);
    ...
    while ($fila = mysqli_fetch_assoc($resultadoSegundaConsulta)) {
      ...
      } else if($senderId = $userLogged["user_id"]){
        echo "<div class='d-flex flex-row justify-content-start'>
          <div>
